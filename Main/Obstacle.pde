public class Obstacle{
  ...
  public void obstruct(){
    if(collision == true){
      ...
      // Left side
      if (((this.position.x - player.getPosition().x <= sprite.width *0.75) && (this.position.x - player.getPosition().x > 0.6 * sprite.width / 2)) && (abs(player.getPosition().y - this.position.y) < sprite.height / 2)){
        player.getPosition().x = position.x - sprite.width *0.75;
      }
